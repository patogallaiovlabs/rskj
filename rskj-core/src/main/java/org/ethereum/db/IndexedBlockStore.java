...
public class IndexedBlockStore implements BlockStore {
    ...
    public synchronized void reBranch(Block forkBlock) {
        ...
        if(maxLevel - bestLine.getNumber() > 1) {
            logger.info("SIMULATION - Rebranching done {} blocks, back from: {} -> to block: {}", maxLevel - bestLine.getNumber(), maxLevel, bestLine.getNumber());
        }
    }
