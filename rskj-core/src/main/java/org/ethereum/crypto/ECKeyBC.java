 ...
 */
public class ECKeyBC implements ECKey {

    private static final Logger logger = LoggerFactory.getLogger(ECKeyBC.class);
     ...
     * @param secureRandom -
     */
    public ECKeyBC(SecureRandom secureRandom) {
    ...
    public ECKeyBC(@Nullable BigInteger priv, ECPoint pub) {
        this.priv = priv;
        if (pub == null) {
     ...
     * @return  -
     */
    public static ECKeyBC fromPrivate(BigInteger privKey) {
        return new ECKeyBC(privKey, compressPoint(CURVE.getG().multiply(privKey)));
    }
     ...
     * @return -
     */
    public static ECKeyBC fromPrivate(byte[] privKeyBytes) {
     ...
     * @return  -
     */
    public static ECKeyBC fromPrivateAndPrecalculatedPublic(BigInteger priv, ECPoint pub) {
        return new ECKeyBC(priv, pub);
    }
     ...
     * @return -
     */
    public static ECKeyBC fromPrivateAndPrecalculatedPublic(byte[] priv, byte[] pub) {
        check(priv != null, "Private key must not be null");
        check(pub != null, "Public key must not be null");
        return new ECKeyBC(new BigInteger(1, priv), CURVE.getCurve().decodePoint(pub));
    }
     ...
     * @return -
     */
    public static ECKeyBC fromPublicOnly(ECPoint pub) {
        return new ECKeyBC(null, pub);
    }
     ...
     * @return -
     */
    public static ECKeyBC fromPublicOnly(byte[] pub) {
        return new ECKeyBC(null, CURVE.getCurve().decodePoint(pub));
    }
    ...
    public ECKeyBC decompress() {
        return new ECKeyBC(priv, decompressPoint(pub));
    }
    ...
    public ECDSASignature sign(byte[] messageHash) {
        ...
        int recId = -1;
        for (int i = 0; i < 4; i++) {
            ECKeyBC k = this.recoverFromSignature(i, sig, messageHash, false);
     ...
     */
    public static ECKeyBC signatureToKey(byte[] messageHash, ECDSASignature signature) throws SignatureException {
        ...
        int recId = header - 27;
        ECKeyBC key = ECKeyBC.recoverFromSignature(recId, signature, messageHash, compressed);
        if (key == null) {
    ...
    public boolean verify(byte[] sigHash, ECDSASignature signature) {
        return this.verify(sigHash, signature, getPubKey());
    }
     ...
     */
    @Nullable
    public static ECKeyBC recoverFromSignature(int recId, ECDSASignature sig, byte[] messageHash, boolean compressed) {
        ...
        return ECKeyBC.fromPublicOnly(q.getEncoded(compressed));
    }
    ...
    public boolean equals(Object o) {
        ...
        if (o == null || !(o instanceof ECKeyBC)) {
            return false;
        }

        ECKeyBC ecKey = (ECKeyBC) o;
