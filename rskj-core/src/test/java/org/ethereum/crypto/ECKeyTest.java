...
public class ECKeyTest {
    ...
    public void testHashCode() {
        Assert.assertEquals(1866897155, ECKeyBC.fromPrivate(privateKey).hashCode());
    }

    @Test
    public void testECKey() {
        ECKeyBC key = new ECKeyBC();
    ...
    public void testFromPrivateKey() {
        ECKeyBC key = ECKeyBC.fromPrivate(privateKey).decompress();
        assertTrue(key.isPubKeyCanonical());
    ...
    public void testPublicKeyFromPrivate() {
        byte[] pubFromPriv = ECKeyBC.publicKeyFromPrivate(privateKey, false);
        assertArrayEquals(pubKey, pubFromPriv);
    }

    @Test
    public void testPublicKeyFromPrivateCompressed() {
        byte[] pubFromPriv = ECKeyBC.publicKeyFromPrivate(privateKey, true);
    ...
    public void testToString() {
        ECKeyBC key = ECKeyBC.fromPrivate(BigInteger.TEN); // An example private key.
    ...
    public void testVerifySignature2() {
        ...
        try {
            ECKeyBC key = ECKeyBC.signatureToKey(HashUtil.keccak256(rawtx), sig);
    ...
    public void testVerifySignature3() throws SignatureException {
        ...
        ECKeyBC key = ECKeyBC.signatureToKey(HashUtil.keccak256(rawtx), tx.getSignature());
    ...
    public void testSValue() throws Exception {
        ...
        for (ECDSASignature signature : sigs) {
            assertTrue(signature.s.compareTo(ECKeyBC.HALF_CURVE_ORDER) <= 0);
        }
    ...
    public void testSignVerify() {
        ECKeyBC key = ECKeyBC.fromPrivate(privateKey);
    ...
    public void testIsPubKeyCanonicalWrongPrefix() {
        ...
        assertFalse(ECKeyBC.isPubKeyCanonical(nonCanonicalPubkey4));
        // Test wrong prefix 2, right length 33
        byte[] nonCanonicalPubkey5 = new byte[33];
        assertFalse(ECKeyBC.isPubKeyCanonical(nonCanonicalPubkey5));
        // Test wrong prefix 3, right length 33
        byte[] nonCanonicalPubkey6 = new byte[33];
        assertFalse(ECKeyBC.isPubKeyCanonical(nonCanonicalPubkey6));
    }
    ...
    public void keyRecovery() throws Exception {
        ...
        key = ECKeyBC.fromPublicOnly(key.getPubKeyPoint());
        boolean found = false;
        for (int i = 0; i < 4; i++) {
            ECKeyBC key2 = ECKeyBC.recoverFromSignature(i, sig, hash, true);
            checkNotNull(key2);
            if (key.equals(key2)) {
                found = true;
                break;
    ...
    public void testSignedMessageToKey() throws SignatureException {
        ...
        ECKeyBC key = ECKeyBC.signatureToKey(messageHash, ECDSASignature.fromComponents(ByteUtil.bigIntegerToBytes(r, 32),
    ...
    public void testEqualsObject() {
        ECKeyBC key0 = new ECKeyBC();
        ECKeyBC key1 = ECKeyBC.fromPrivate(privateKey);
        ECKeyBC key2 = ECKeyBC.fromPrivate(privateKey);
    ...
    public void decryptAECSIC(){
        ECKeyBC key = ECKeyBC.fromPrivate(Hex.decode("abb51256c1324a1350598653f46aa3ad693ac3cf5d05f36eba3f495a1f51590f"));
