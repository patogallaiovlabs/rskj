...
from log_processor import coinbase_labels
...
def plot_miner_blocks():
    ...
    total_main_blocks = sum(main_blocks)
    total_sibling_blocks = sum(sibling_blocks)
    total_blocks = total_main_blocks + total_sibling_blocks

    # Add labels to the bars with percentages
    for bar in bars1:
        height = bar.get_height()
        if total_main_blocks > 0:
            percentage = (height / total_main_blocks) * 100
    ...
    for bar in bars2:
        height = bar.get_height()
        if total_sibling_blocks > 0:
            percentage = (height / total_sibling_blocks) * 100
            ax6.text(bar.get_x() + bar.get_width() / 2, height, f'{height}\n({percentage:.1f}%)', ha='center', va='bottom')
        else:
            ax6.text(bar.get_x() + bar.get_width() / 2, height, f'{height}', ha='center', va='bottom')
...
def plot_new_blocks_vs_reorgs():
    ...
    total = new_block_count + reorg_count
    ...
    for bar in bars:
        ...
        if total > 0:
            ...
            ax7.text(bar.get_x() + bar.get_width() / 2, height, f'{height}\n({percentage:.1f}%)', ha='center', va='bottom')
        else:
