...
def update_plot():
    ...
    miners = [coinbase_labels.get(miner, miner) for miner in average_block_times.keys()]
    ...
    for miner, times in miner_times.items():
        if times:  # Only plot if there are block times
            ax3.hist(times, bins=bins, alpha=0.7, label=coinbase_labels.get(miner, miner), histtype="step", linewidth=1.5)
...
def process_line(line):
    ...
    if match:
        ...
        if miner not in coinbase_labels:
